...
namespace nl {
	...
	public:
		...
		void set_state(states... s) {
			...
			std::array<size_t, sizeof...(s)> sts{ s... };
			for (auto& st : sts) {
		...
		template<typename T, std::enable_if_t<std::disjunction_v<std::is_same<typename T::value_type, std::uint8_t>, std::is_same<typename T::value_type, std::int8_t>>,int> = 0>
		...
		tuple_buffer & read(T & value) 
		{
			...
			std::copy_n(std::next(mBuffer.begin(), mReadHead), read_size, std::back_insert_iterator<decltype(value)>{value});
			mReadHead += read_size;
			return (*this);
	...
	//reads from the buffer into the tuple
	 ...
	 void read_tuple_buffer(tuple_t& tuple, tuple_buffer<tuple_t>& buffer) {
		//overides the state, that is in the tuple, 
		 nl::detail::loop<std::tuple_size_v<tuple_t> -1>::template do_tbuffer_read<tuple_t, tuple_buffer<tuple_t>>(tuple, buffer);
	 }
	 ...
	 {
		 buffer.reset();
		 nl::detail::loop<std::tuple_size_v<tuple_t> -1>::template do_tbuffer_write<tuple_t, tuple_buffer<tuple_t>>(tuple, buffer);
	 }
